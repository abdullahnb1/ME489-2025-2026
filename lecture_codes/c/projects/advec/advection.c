/* 
  ...
  1D/2D Linear Advection Solver in C
  ----------------------------------
  This program solves the **linear advection equation** of the form:
 
      ∂q/∂t + u · ∇q(x, y) = 0
 
  where:
    - q(x, y, t) represents a transported scalar quantity (e.g., density, temperature, or concentration),
    - u = (u_x, u_y) is the velocity vector that defines the direction and speed of advection,
    - ∇q denotes the spatial gradient of q.
 
  Governing Equation:
  -------------------
  In component form for 2D:
      ∂q/∂t + u_x ∂q/∂x + u_y ∂q/∂y = 0
 
  The equation represents **pure advection** (or convection) — 
  meaning the scalar field q is transported by a constant velocity field u 
  without any diffusion or source terms.
 
  Numerical Discretization:
  -------------------------
  The computational domain is discretized using a **uniform Cartesian grid**
  consisting of NX × NX evenly spaced points. 
  The grid spacing (Δx, Δy) is determined by the domain size and number of grid points.
 
  The **first and last points** in each direction correspond to **boundary nodes**,
  where Dirichlet or periodic boundary conditions can be applied depending on the problem setup.
 
  The spatial derivatives (∂q/∂x and ∂q/∂y) are approximated using 
  **first-order upwind finite difference schemes**, which are stable for 
  advection-dominated problems but introduce numerical diffusion.
 
  Temporal Integration:
  ---------------------
  Time advancement is performed using a simple **explicit Euler method**:
      q^{n+1} = q^n - Δt * (u_x ∂q/∂x + u_y ∂q/∂y)
 
  The time step Δt must satisfy the **CFL (Courant–Friedrichs–Lewy) condition** 
  to ensure numerical stability:
      CFL = (|u_x| Δt / Δx + |u_y| Δt / Δy) ≤ 1
 
  Algorithm Overview:
  -------------------
  1. Initialize the computational grid and set the initial condition for q(x, y, t=0).
  2. Specify the advection velocity components (u_x, u_y).
  3. Apply boundary conditions (Dirichlet, Neumann, or periodic).
  4. Compute spatial derivatives using upwind differences.
  5. Update the field q at each time step using the explicit time-stepping scheme.
  6. Continue until the desired final time is reached.
  7. Output the results for visualization or analysis.
 
  Notes:
  ------
  - This is a simplified pedagogical example — higher-order spatial
    (e.g., MUSCL) can be implemented for improved accuracy.
  - The method is applicable to both 1D and 2D problems by modifying the spatial operators.
  - Periodic boundaries are commonly used for testing advection accuracy.
 
  Summary:
  --------
  The goal of this code is to illustrate the numerical treatment of **linear advection** 
  using **finite differences**, focusing on practices on C programming language.

  To build the code :
  -------------------- 
  gcc -I -o advection.o advection.c -lm

  post processing:
  ---------------
  first do following before regular operations
    - Go to Filters → Alphabetical → Table To Points
    - Apply the filter-> Filters → Alphabetical → Delaunay2D
...
int main ( int argc, char *argv[] ){
  ...
  // Read input file for given name 
  // time variables 
...
for(int j=0; j<msh->NY; j++){
    for(int i=0; i<msh->NX; i++){
      ...
      double hip1 = msh->h[4*idn + 0]; 
      double him1 = msh->h[4*idn + 2];
      double hjp1 = msh->h[4*idn + 1];
      double hjm1 = msh->h[4*idn + 3];
      ...
      // Update q i.e. q = q + rk4b(stage)*resq
...
// Create periodic connectivity
  msh.h = (double *)malloc(4*msh.Nnodes*sizeof(double)); 
  for(int j=0; j<msh.NY; j++){
    for(int i=0; i<msh.NX; i++){
      
      const int idn = j*msh.NX + i;
      
      const int eE = msh.N2N[4*idn + 0];  
      const int eN = msh.N2N[4*idn + 1];  
      const int eW = msh.N2N[4*idn + 2];  
      const int eS = msh.N2N[4*idn + 3];  
      
      msh.h[4*idn + 0] = fabs(msh.x[eE ] - msh.x[idn]);  
      msh.h[4*idn + 1] = fabs(msh.y[eN ] - msh.y[idn]);  
      msh.h[4*idn + 2] = fabs(msh.x[idn] - msh.x[eW ]);  
      msh.h[4*idn + 3] = fabs(msh.y[idn] - msh.y[eS ]);  

      if(j==0){msh.h[4*idn + 3]           = msh.h[4*idn + 1];}
      if(j==(msh.NY-1)){msh.h[4*idn + 1]  = msh.h[4*idn + 3];}
      if(i==0){msh.h[4*idn + 2]           = msh.h[4*idn + 0];}
      if(i==(msh.NX-1)){msh.h[4*idn + 0]  = msh.h[4*idn + 2];}
    }
  }
  return msh; 
}
